...
# (((^_(((/(((_/
#
# Model Presentation Again (shhh... he's sleeping)
...
#' ## Load the data 
#' 
#' We'll use an example from the first lab session of the course. This is a 
#' simple little data set for all 28 European Union member states with
#' various economic indicators of interest. These are income inequality (`gini`), 
#' FDI net inflows as a percentage of GDP (`fdipgdp`), exports as a percentage 
#' of GDP (`exppgdp`), the real effective exchange rate (`reer`), tax revenue 
#' as a percentage of GDP (`taxrevpgdp`), GDP in constant 2015 USD (`gdp`), and 
#' population size (`pop`). The `wp` variable communicates whether the European 
#' Union state was in the Warsaw Pact or not. Former republics of the Soviet 
#' Union (e.g. Estonia) and Poland, for example, would both be 1s. France and 
#' the United Kingdom would both be 0. You can also see my plea from the first
#' lab script to take this as a cue to lean more about 
#' [how to use the `{WDI}` package](https://svmiller.com/blog/2024/10/make-simple-cross-sectional-world-bank-data-wdi/))

Data <- haven::read_dta("http://svmiller.com/extdata/eu2019.dta")

#' I'll be honest that I don't know what hypothesis I would care to ask about
#' what follows, but what follows is a minimal working example for the sake
#' of this lab session. I'm building toward a regression of exports as a % of
#' GDP on the real effective exchange rate, the Warsaw Pact variable, the GINI
#' measure for income inequality, FDI as a percentage of GDP, and logged GDP 
#' per capita. Of those, I just have to create the log-transformed GDP per capita
#' variable as follows.

Data %>%
  mutate(gdppc = gdp/pop,
         ln_gdppc = log(gdppc)) -> Data

# Alternatively:
# Data %>% mutate(ln_gdppc = log(gdp/pop)) -> Data

#' Before going any further, it would be useful to understand some of the 
#' variation around the thing we want to explain (exports as a percentage of
#' GDP in this application).

Data %>%
  ggplot(., aes(exppgdp)) + 
  geom_histogram(fill = g_c("su_water"), color = "black") +
  # Below: percent_format() likes to assume you're working with proportions
  # You're not here, so set this scale argument to 1 (it's 100 by default).
  scale_x_continuous(labels = scales::percent_format(scale = 1))

#' If I were interested in a hypothesis of the differences between the EU member
#' states previously in/covered by the Warsaw Pact and those that were not, I 
#' could try a preliminary comparison like this by way of a box plot.

Data %>%
  mutate(wpcat = ifelse(wp == 1, "Warsaw Pact", "Not Warsaw Pact")) %>%
  ggplot(., aes(wpcat, exppgdp)) +
  geom_boxplot(fill = g_c("su_water")) +
  coord_flip() +
  # Below: percent_format() likes to assume you're working with proportions
  # You're not here, so set this scale argument to 1 (it's 100 by default).
  # This time: it's the y-axis, notwithstanding coord_flip()
  scale_y_continuous(labels = scales::percent_format(scale = 1))

#' Our preliminary assessment of such a graph is exports are generally a higher
#' percent of GDP in the Warsaw Pact states than they are in the states that
#' were not previously in or covered by the Warsaw Pact. We certainly see some
#' runaway observations, though, and greater variation in the distribution for
#' the non-Warsaw Pact states.

Data %>%
  arrange(exppgdp) %>%
  group_split(wp)

#' That runaway observation for the not-Warsaw Pact states is Luxembourg (206%)
#' and that minimum observation for the Warsaw Pact states is Romania (40.1%).
#' I was not anticipating doing this prior to this point in the lab script, but
#' let's log

M1 <- lm(exppgdp ~ reer + wp + log(gdppc) + gini + fdipgdp, Data)
M2 <- lm(log(exppgdp) ~ reer + wp + log(gdppc) + gini + fdipgdp, Data)
summary(M1)
