...
\tightlist
\item
  \(m\) (eller \(b\) på USA) is the intercept: the observed \(y\) when
  \(x\) = 0.
\item
  \(k\) (eller \(m\) på USA) is the familiar ``rise over run'',
  measuring the amount of change in \(y\) for a unit change in \(x\).
\end{itemize}
\end{frame}
...
The \(b\) is the \textbf{regression coefficient} that communicates the
...
\begin{frame}{The DV and IV are Both Log-Transformed}
...
\tightlist
\item
  Alternatively, less helpfully: a unit increase \(log(x)\) increases
  \(log(y)\) by an estimated .283.
\end{itemize}
...
\tightlist
\item
  Alternatively, with more precision: \((1.01^{.763}-1)*100 = .762\)\%
  change in tourism arrivals for 1\% increase in GDP per capita.
\end{itemize}
